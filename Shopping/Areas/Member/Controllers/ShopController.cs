...
using Shopping.Areas.Member.Models.Shop;
...
namespace Shopping.Areas.Member.Controllers
{
    ...
    public class ShopController : Controller
    {
        ...
        public ShopController(UserManager<AppUser> userManager, IProductService productService, Context c)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> ShopAdd(ShopViewModel p)
        {
            var user = await _userManager.FindByNameAsync(User.Identity.Name);
            var userid = user.Id;
          
            if (p.ShopSmallImage != null)
            {
                var Extension = Path.GetExtension(p.ShopSmallImage.FileName);
                var newImageName = Guid.NewGuid() + Extension;
                var Location = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Shopping/ShopSmallImg/" + newImageName);
                var Stream = new FileStream(Location, FileMode.Create);
                p.ShopSmallImage.CopyTo(Stream);
                user.ShopSmallImage = newImageName;
            }
            if (p.ShopBigImage != null)
            {
                var Extension2 = Path.GetExtension(p.ShopBigImage.FileName);
                var newImageName2 = Guid.NewGuid() + Extension2;
                var Location2 = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Shopping/ShopBigImg/" + newImageName2);
                var Stream2 = new FileStream(Location2, FileMode.Create);
                p.ShopBigImage.CopyTo(Stream2);
                user.ShopBigImage = newImageName2;
            }
            else
            {
                user.ShopBigImage = null;
            }
            user.ShopName = p.ShopName;
            user.ShopDescription = p.ShopDescription;
            user.ShopDate = Convert.ToDateTime(DateTime.Now.ToShortDateString());
            user.ShopStatus = "Active";
            user.ShopPhone = p.ShopPhone;
            user.ShopConfirmed = "Təstiqlənməyib";
            var result = await _userManager.UpdateAsync(user);
            if (result.Succeeded)
            {

                return RedirectToAction("InShop", "Shop", new { id =userid});
            }
            return View(p);
        ...
        [HttpPost]
        public async Task<IActionResult> Update(ShopViewModel p)
        {
            var user = await _userManager.FindByNameAsync(User.Identity.Name);
            var userid = user.Id;
            ...
            user.ShopDescription = p.ShopDescription;
            user.ShopDate = Convert.ToDateTime(DateTime.Now.ToShortDateString());
            user.ShopStatus = user.ShopStatus.ToString();
            user.ShopPhone = p.ShopPhone;
        ...
        public JsonResult Category1json(int ID)
        {
            ...
            var categoryl = _c.Categories.Where(w => w.Category2Id == ID).ToList();
        ...
        {
            var user = await _userManager.FindByNameAsync(User.Identity.Name);
            ViewBag.ShopConfirm = user.ShopConfirmed;
            return View();
        }
        [HttpPost]
        public async Task<IActionResult> Confirmed(ShopConfirmedViewModel model)
        {
            var Shopconfirmed = await _userManager.FindByNameAsync(User.Identity.Name);
            Shopconfirmed.ShopConfirmed = "Yoxlanılır";
            Shopconfirmed.ShopPhone = model.Phone;
            var result = await _userManager.UpdateAsync(Shopconfirmed);
            if (result.Succeeded)
            {
                return View();
            }
